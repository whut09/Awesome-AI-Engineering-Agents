...
当前，大模型已经在文本生成、图像与视频生成、代码编写等领域取得快速进展。
以 **Codex**、**OpenHands**、**WorkBuddy** 为代表的通用编程智能体和工作助手，
已经能够完成或显著加速这些领域中的大量任务。AI 对软件开发和数字内容岗位的
影响已经十分明显，但在依赖专业物理知识、EDA/CAD 工具、企业私有数据、形式验证、
制造约束及实体产品责任的工程岗位中，其渗透速度并不一致。

本项目从智能硬件类公司的岗位构成切入，重点关注大模型和智能体仍处于从研究原型
走向实际工程流程阶段的四类岗位：**光学工程**、**硬件与 PCB 设计**、
**结构与 CAD 工程**和 **FPGA 工程**。收录范围包括镜头设计、光学仿真、原理图生成、
PCB 布局布线、图纸检查与审核、结构与受力分析、结构仿真、拓扑和壁厚优化、
RTL 生成、功能验证、HLS、综合、时序收敛以及 FPGA 布局布线等具体工作。

本项目不仅整理相关论文和开源项目，也希望持续跟踪 AI 对这些工程岗位的渗透趋势：
哪些任务已经出现可靠的智能体与开放实现，哪些仍处于研究阶段，以及哪些环节仍然
离不开工程师的专业判断和责任确认。因此，清单优先收录 **2020 年及以后**的论文和
仍在积极维护的开源项目，并按照实际工程工作流组织，而不是简单按照模型类别罗列。
